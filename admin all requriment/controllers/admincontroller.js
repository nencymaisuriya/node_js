...
const viewAdminPage = async (req, res) => {
    try {
       ...
       records = records.filter((data)=> data.id!=cureentuser.id);
...
const editAdmin = async (req, res) => {
    try {
        ...
        if (req.file) {
            const oldPath = data.image; // e.g., "/uploads/xyz.jpg"
...
const passchangepage = async (req, res) => {
    const currentAdmin = req.user;
    res.render('passwordChange', { currentAdmin, success: req.flash('success'), error: req.flash('error') });
};
